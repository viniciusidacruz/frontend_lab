...
export const PlatformDonation = ({ platforms }: PlatformDonationProps) => {
  const handlePlatformClick = (platform: Platform) => {
    capturePostHogEvent(DONATE_EVENTS.PLATFORM_CLICKED, {
    ...
    <div className="space-y-4">
      {platforms.map((platform) => (
        ...
        >
          ...
          <ExternalLink className="w-5 h-5 text-zinc-500 group-hover:text-blue-400 transition-colors shrink-0" />
        </Link>
      ))}
    </div>
  );
